export function PostWebinarAudienceFit() {
  const goodFit = [
    "czujesz, że chcesz żyć inaczej, nawet jeśli nie masz jeszcze wszystkiego dokładnie poukładanego",
    "dużo już rozumiesz albo dopiero zaczynasz swoją pracę ze sobą, ale chcesz przekładać ją na codzienne życie",
      ...
      <div className="container mx-auto px-6 max-w-5xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-24 mb-16 md:mb-20">
          ...
          <div className="flex flex-col">
            <h3 className="font-heading text-xl md:text-[22px] font-bold text-[#C29B57] tracking-wider uppercase mb-8 md:mb-10">
            ...
            <ul className="flex flex-col">
              {goodFit.map((text, idx) => (
                <li key={idx} className="flex items-start py-6 border-b border-slate-100 last:border-0">
                  <span className="w-2 h-2 rounded-full bg-[#C29B57] mt-2.5 mr-5 shrink-0"></span>
                  <span className="text-[18px] md:text-[20px] text-slate-800 font-light leading-[1.8]">{text}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* BLOK 2 */}
          <div className="flex flex-col">
            <h3 className="font-heading text-xl md:text-[22px] font-bold text-slate-700 tracking-wider uppercase mb-8 md:mb-10">
              TEN PROGRAM NIE BĘDZIE DOBRYM WYBOREM, JEŚLI:
            </h3>
            <ul className="flex flex-col">
              {badFit.map((text, idx) => (
                <li key={idx} className="flex items-start py-6 border-b border-slate-100 last:border-0">
                  <span className="w-2 h-2 rounded-full bg-slate-500 mt-2.5 mr-5 shrink-0"></span>
                  <span className="text-[18px] md:text-[20px] text-slate-700 font-light leading-[1.8]">{text}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        {/* DISCLAIMER */}
        <div className="pt-10 border-t border-slate-200 text-center max-w-3xl mx-auto">
          <p className="text-base md:text-[17px] text-slate-800 font-medium">
